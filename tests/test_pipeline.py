...
from salary_analysis.pipeline import load_dataset, missing_summary, run_pipeline


def test_load_dataset_normalizes_columns(tmp_path):
    dataset = tmp_path / "sample.csv"
    pd.DataFrame({"Job Title": ["Data Engineer"], "Salary USD": [100]}).to_csv(dataset, index=False)
    df = load_dataset(dataset)
    assert list(df.columns) == ["job_title", "salary_usd"]
...
def test_run_pipeline_writes_outputs(tmp_path):
    ...
    pd.DataFrame({"salary": [1, 2]}).to_csv(dataset, index=False)
